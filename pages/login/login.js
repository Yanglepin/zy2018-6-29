...
Page({
  ...
  sendMsg: function () {
    let that=this;
    // console.log("点击登录" + that.data.ImgUrl)
    wx.request({
...
//登录
  onSubmit: function () {
    ...
    wx.request({
      ...
      success: function (res) {
        // console.log("00000" + JSON.stringify(res.data))
        // console.log("111" + JSON.stringify(app.globalData.logincode))
        if (res.data.code > 0) {
          app.globalData.logincode = res.data.uid
          wx.showToast({
            title: '验证成功',
            icon: 'success'
          })
